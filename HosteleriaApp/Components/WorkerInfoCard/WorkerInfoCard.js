...
const WorkerInfoCard = ({
  ...
  const openInstagram = () => {
    if (instagram && typeof instagram === "string" && instagram.trim() !== "") {
      ...
      console.error(
        "El nombre de usuario de Instagram no está definido o es inválido"
      );
    }
  };
    ...
    if (linkedin && typeof linkedin === "string" && linkedin.trim() !== "") {
      ...
      Linking.canOpenURL(linkedinUrl).then((supported) => {
        if (supported) {
          ...
          console.error("No se puede abrir linkedin");
        }
      });
    } else {
      console.error("La direccion de linkedin no es correcta");
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.infoContainer}>
        ...
        <Text style={styles.anydeinici}>
          {anydeinici ? ` Inici: ${anydeinici}` : "-"}
        </Text>
      </View>
      ...
      <View style={styles.buttonContainer}>
        ...
        {linkedin ? (
          <TouchableOpacity style={styles.boton} onPress={openLinkedin}>
            <Ionicons name="logo-linkedin" size={30} color="#fff" />
            <Text style={styles.botonText}>Linkedin</Text>
          </TouchableOpacity>
        ) : null}
